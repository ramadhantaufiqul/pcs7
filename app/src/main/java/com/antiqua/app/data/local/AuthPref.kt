package com.antiqua.app.data.local

import android.content.Context
import android.content.SharedPreferences
import com.antiqua.app.data.model.ActionState
import com.antiqua.app.data.model.AuthUser
import com.antiqua.app.util.getObject
import com.antiqua.app.util.putObject
...
class AuthPref(val context:Context) {
    ...
    suspend fun register(user: AuthUser): ActionState<AuthUser> {
        return if (user.email.isBlank() || user.password.isBlank()) {
            ActionState(message = "Email dan password tidak boleh kososng", isSuccess = false)
        } else {
            authUser = user
